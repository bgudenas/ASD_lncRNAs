...
sft=pickSoftThreshold(datExpr0, powerVector=powers, verbose=5, networkType="signed", corFnc = "bicor",corOptions = list(use = 'p', maxPOutliers = 0.1), blockSize = 27000)
...
heatmap.2(ad_p, cellnote=OR_filter, trace="none", main="Module Gene Set Enrichment", RowSideColors=Modules_cap, notecol="black",key=TRUE, col=my_palette, symm=F, symkey=F, symbreaks=F, key.xlab="-Log( FDR adjusted p-value )", notecex =1.5, margins =c(13,8), breaks = seq(0, 15, length.out =400))
```

Next we will validate the co-expression modules to ensure they are truly co-expressed
...
```
Now we will analyze the average ASD cortex Log2 Fold-change of the developmental co-expression module gene lists. Any gene not considered differentially expressed has a log2 fold-change of 0 indicating modules not dysregulated in ASD should approximate 0.
...
text(x= df.bar, 0, Sig_P, cex =2, pos=Sig_pos, col="white", offset = 3)
...
knitr::opts_chunk$set(comment=NA, fig.width=16, fig.height=6)
library(ggplot2)
library(gridExtra)
interesting_mods = c("brown","black","blue","darkorange","red","turquoise","tan") ## all modules enrriched for DE_lncRNAs and/or both ASD gene sets
plot_vec = vector("list",length=length(interesting_mods))
names(plot_vec) = interesting_mods
...
for (mod in interesting_mods){
    ...
    p1 = ggplot(mod_terms, aes( y=-log10(Pvalue), x=reorder(Term, Pvalue), fill= type)) +
        ...
        coord_flip()+
        ylab(expression(-Log[10]~Pvalue)) + xlab( "GO Term") +ggtitle(paste(paste0(toupper(substr(mod, 1, 1)), tolower(substring(mod, 2))), "Functional Enrichment") )+
    ...
    p2= ggplot(df, aes(x = Months, y=Expression),
        ...
        ggtitle(label =  paste0(toupper(substr(mod, 1, 1)), tolower(substring(mod, 2)) )) +
        geom_vline(xintercept =  10, col="blue", lwd=1)

    grid.arrange(p1, p2, ncol=2, nrow =1, widths = c(3.5,1))
}
```
CNV data from SFARI is in cytoband format (ex: 3q29) from different genome assemblies so we will need to create a function to convert all cytoband coordinates into hg28 genomic coordinates.
...
# test that cyto_converter works on input cytoband file
cyto_test = cyto_converter(cytobands$Band, cytobands)
table(cyto_test$Cytoband == cytobands$Band)

```
Now we will use cyto_converter to convert SFARI ASD CNVs into genomic coordinates and then calculate overlaps with the differentially expressed lncRNAs.

```{r CNV ranking, warning=FALSE}
load(file="./Data/Post_geneinfo_network.RData")
